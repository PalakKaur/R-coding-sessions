...
library(MASS)
data("Cars93") # we will be using this dataset
...
my_function <- function(col){     
  if(is.numeric(col)==TRUE){
    mean_med <- c(mean(col), median(col))
    return(mean_med)}else{print("Variable is not numeric!")}
}
...
## Exercise: time between each phone call on a festival
